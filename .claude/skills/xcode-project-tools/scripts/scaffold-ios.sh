#!/bin/bash
# Scaffolds a new iOS project from templates
# Creates a modern Xcode project with workspace structure and SPM package
#
# Usage: bash scaffold-ios.sh <project-name> <output-path> [bundle-id] [display-name]
#
# Parameters:
...
#
# Examples:
#   bash scaffold-ios.sh MyApp ~/Projects
#   bash scaffold-ios.sh MyApp ~/Projects com.example.myapp
#   bash scaffold-ios.sh MyApp ~/Projects com.example.myapp "My App"

# Validate required parameters
if [ -z "$1" ]; then
  echo "Error: Project name required"
  echo "Usage: bash scaffold-ios.sh <project-name> <output-path> [bundle-id] [display-name]"
  exit 1
fi
